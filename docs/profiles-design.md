...
Status: **Implemented.** The follow-up "POST /api/runs profile-id migration" called out in PR #43's status finished with the A2 trio — `wt/runs-profile-additive` (PR #70, A2-P1 additive), `wt/runs-profile-ui` (PR #71, A2-P2 Playground sender), `wt/runs-profile-breaking` (PR #72, A2-P3 BREAKING). `routes/__init__.py` is now at contract v4: `RunCreate.provider` and `RunCreate.model` are removed; `RunCreate.profile_id: str` is the single required dispatch selector. `playground/dispatch.py::_default_client_factory` / `set_client_factory` / `ClientFactory`, `aitap.deep.client.register_provider` / `get_client` / `_REGISTRY`, `aitap.deep.openai_client.OpenAIClient`, and `aitap.deep.anthropic_client.anthropic_factory` (+ the `ANTHROPIC_BASE_URL` env-var hook) were all deleted; `aitap scan --deep` no longer carries a legacy fallback either. The remaining client construction surface is `aitap.deep.factory.get_client_for_profile_config` (and its API-twin `get_client_for_profile`).

Note on the residual `aitap.secrets` legacy API: PR #43's Status text said the legacy provider-keyed `secrets.get_key` / `set_key` / `delete_key` / `key_status` were "left in place" because `POST /api/runs` and `aitap scan --deep` still called them. After A2-P3 every production caller is gone (the `_ALLOWED_FILES` allow-list in `tests/unit/test_secrets_import_discipline.py` shrinks to just `secrets.py` itself), so those functions are now **unreachable from production code** — they're kept only because `tests/unit/test_secrets_no_leak.py` and `tests/unit/test_secrets_profiles.py` still exercise the legacy surface to lock down log-redaction and on-disk format guarantees. A follow-up worktree (or a future A2-P4) can delete the legacy API and its remaining tests together; doing it in A2-P3 was deferred to keep the breaking-change blast radius bounded.

Previously implemented in PR #43 (2026-06-02): the bounded cleanup worktree removed the legacy contract types (`ProviderKeyStatus` / `SetKeyRequest` / `TestKeyResponse` / `SettingsUpdate`), the `SettingsResponse.keys` field, the four legacy routes, and the orphan `src/aitap/ui/src/api/settings-keys.ts` TypeScript wrapper; `openapi.json` and the generated TS client were regenerated to match. All four open questions resolved (see Decision log at the bottom). Note: the `Profile` data model below lists `key_source` as `Literal["keyring", "fallback", "env", "none"]`, but the implemented `routes/__init__.py` Profile narrows to `"keyring" | "fallback" | "none"` (profile-id keys don't come from env vars — `env` was a legacy provider-keyed concept).
