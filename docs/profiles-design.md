...
Status: **Implemented in PR #43 (2026-06-02).** All four worktrees shipped — `wt/profile-model` (PR #38), `wt/profile-client` (PR #40), `wt/profile-ui` (PR #42), `wt/profile-cleanup` (PR #43). The multi-provider redesign is the current contract; `routes/__init__.py` is at v3. The bounded cleanup worktree removed the legacy contract types (`ProviderKeyStatus` / `SetKeyRequest` / `TestKeyResponse` / `SettingsUpdate`), the `SettingsResponse.keys` field, the four legacy routes, and the orphan `src/aitap/ui/src/api/settings-keys.ts` TypeScript wrapper; `openapi.json` and the generated TS client were regenerated to match. The `aitap.secrets` legacy provider-keyed API, the `register_provider` / `get_client` factory + `OpenAIClient`, `playground/dispatch.py::_default_client_factory`, and `config_io.py`'s legacy `provider:` block are left in place because the `POST /api/runs` path (`RunCreate.provider`) and `aitap scan --deep` L2 enrichment still call through them — a follow-up worktree migrates that surface to profile-id dispatch. All four open questions resolved (see Decision log at the bottom). Note: the `Profile` data model below lists `key_source` as `Literal["keyring", "fallback", "env", "none"]`, but the implemented `routes/__init__.py` Profile narrows to `"keyring" | "fallback" | "none"` (profile-id keys don't come from env vars — `env` was a legacy provider-keyed concept).
