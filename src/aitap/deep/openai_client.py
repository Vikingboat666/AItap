...
# A2-P3 (contract v4): the legacy ``OpenAIClient`` class +
# ``register_provider("openai", ...)`` registration are gone.
# ``OpenAICompatClient`` below is the single OpenAI-protocol client
# for every configured profile (the canonical OpenAI endpoint is just
# another profile with ``base_url="https://api.openai.com/v1"``).
# Helpers below are still shared between OpenAICompatClient and the
# pricing/estimation code paths.
...
# --------------------------------------------------------------------------- #
# OpenAICompatClient — the single OpenAI-protocol client (multi-provider)     #
# --------------------------------------------------------------------------- #
#
# A2-P3 (contract v4) removed the legacy :class:`OpenAIClient` + its
# ``register_provider("openai", ...)`` registration. ``OpenAICompatClient``
# is now the only OpenAI-protocol surface; the canonical OpenAI endpoint
# is just another profile (``base_url="https://api.openai.com/v1"``).
# See ``docs/profiles-design.md`` §"Backend architecture / LLM client
# construction".
