...
**Store — persist path resolution + orphan YAML cleanup (two real bugs)**
- PR #54 (`wt/store-persist-fixes`) — two bugs the web-playground eval surfaced when re-scanning cc-project. (a) `_persist_if_initialised` constructed `Settings()` bare; pydantic-settings then defaulted `project_root` to `Path.cwd()`, which the user's invocation (`uv --directory <aitap_repo> run aitap scan <user_project>`) had already moved to the wrapper's project. Persistence tried to write into `<aitap_repo>/.aitap/`, which didn't exist, and the report came back with "warning: failed to persist scan to .aitap/: [Errno 2]". Resolution order is now: `AITAP_PROJECT_ROOT` env var (the documented override and what the integration test already relied on) → `ScanResult.project_root` (the scan target) → `Path.cwd()` (programmatic case). (b) When a site's content fingerprint changes between scans, the site id changes too and so does the YAML filename; the prior YAML stayed on disk forever, doubling up the inventory by name — the cc-project HEAVEN_WORLD_RULES eval hit this exact pattern after the `dedent(...)` PR #52 changed the resolved-text fingerprint. `persist_scan_result` now snapshots the existing prompt / pipeline YAML filenames before the write pass, records what it wrote, and unlinks the difference. `PersistReport` gains `prompts_removed` / `pipelines_removed` counters so the CLI summary line documents the cleanup. +3 integration tests (893 backend total): one pins the cwd-mismatch regression (no env var, wrapper CWD has no `.aitap/`), one plants a stale YAML and asserts it disappears after re-persist with the report counter equal to 1, one verifies a clean re-scan with no orphans doesn't crash and reports zero removals.

**Server — SPA fallback no longer masks stale bundle 404s as 200 HTML**
- PR #55 (`wt/spa-assets-404`) — bug-fix surfaced live during the web-playground eval. After each frontend code change `vite build` writes hashed bundle files (``index-CmveJA5d.js`` etc) into ``src/aitap/server/static/assets/`` and ``index.html`` references the new hash. A user whose browser still cached the prior ``index.html`` re-requested the **old** hashed bundle URL — and the SPA fallback in ``_SpaStaticFiles`` (PR #36) handed back ``index.html`` for the missing asset because the request path didn't match the API / docs deny-list. The browser then tried to execute the HTML body as JavaScript, the page rendered blank, and there was no console error a user could act on. Fix: extend the deny-list to include ``assets/``, so stale ``/assets/<old-hash>.js`` requests get a real ``404`` — which makes the browser invalidate its cached ``index.html`` and pick up the new hash on the next reload. The regression test added in PR #36 (``test_spa_fallback_serves_index_html_for_client_routes``) grows three new asserts that hit ``/assets/index-CM3n9w6E.js`` (a known-stale hash) plus the matching ``.css`` and ``.js.map`` siblings and confirms each returns ``404``. No new test file — kept the fix in the same place as the rule it modifies.

**Deep scan — legacy ``anthropic`` factory honours ``ANTHROPIC_BASE_URL``**
- PR #58 (`wt/deep-anthropic-base-url`) — `wt/profile-cleanup` (PR #43) left `aitap scan --deep` riding the legacy provider-keyed factory (`get_client("anthropic", ...)`), which constructed `AnthropicClient` without a `base_url` argument — so the L2 enrichment path was permanently pinned to `https://api.anthropic.com` even though `AnthropicClient` itself had grown a `base_url` constructor kwarg in PR #40. This blocked the very practical workflow of pointing L2 at an Anthropic-protocol-compatible gateway: most directly, DeepSeek's `https://api.deepseek.com/anthropic` endpoint (the user's eval target — DeepSeek is much cheaper than Anthropic for the per-prompt purpose summarisation L2 does), but the same hole would have blocked a self-hosted Anthropic gateway or a future regional endpoint just as completely. New `_anthropic_factory(model, key)` replaces the one-line lambda in `aitap.deep.anthropic_client`: when `ANTHROPIC_BASE_URL` env var is set and non-empty, the factory forwards it to `AnthropicClient(... , base_url=env)`; otherwise the constructor's default (`https://api.anthropic.com`) keeps every existing caller byte-for-byte unchanged. Whitespace-only env vars (`""`, `"   "`, `"\t\n"`) are treated as unset so a copy-paste accident doesn't quietly break the legacy path. The env override deliberately lives in the *factory*, not the `AnthropicClient` constructor — direct constructor callers and the new `get_client_for_profile` path both pass `base_url` explicitly, and an inner override would silently shadow their choice. +8 unit tests (`tests/unit/test_anthropic_factory_env_base_url.py`) pinning the contracts: the first three (no env → default; env set → propagated verbatim; whitespace-only → unset) plus the new strip case (surrounding whitespace around an otherwise valid URL → stripped, then used) all routed through the public `get_client("anthropic", ...)` entry point so the test path mirrors how `aitap scan --deep` actually reaches the factory; one isolation test (explicit constructor `base_url=` wins over env) deliberately uses the direct constructor to prove env doesn't leak into other call paths; and an end-to-end test bridges factory → `AnthropicClient` → `AsyncAnthropic` constructor with a recording fake SDK, so a future refactor that drops the value at any link in the chain trips a single visible test. Factory function is `anthropic_factory` (public, no underscore) so monkeypatch / extension consumers can reach it without crossing the private-symbol contract. Tech-review pass added the SDK-env-var conflict note to the factory docstring: the Anthropic Python SDK reads `ANTHROPIC_BASE_URL` itself when constructed with `base_url=None`, but `AnthropicClient` always forwards a non-`None` default — meaning aitap is the sole interpreter of the variable. This is deliberate (single owner = predictable profile path), documented in the factory docstring + CHANGELOG, and leaves the door open to a separate `AITAP_ANTHROPIC_BASE_URL` namespace if direct-constructor env semantics ever become desirable. Backend tests: 909 → **917**. No new docstring in `CLAUDE.md`, no contract change, no UI surface. Follow-up still flagged: the full migration of `aitap scan --deep` onto the profile-keyed `get_client_for_profile` API (which would let DeepSeek, Moonshot, Groq, etc. all flow through the new `OpenAICompatClient` path natively, without leaning on Anthropic-protocol compatibility) is the cleaner long-term answer. The symmetric OpenAI hole (`OpenAIClient` legacy factory also pinned to `api.openai.com`) is deliberately **not** patched in this PR — OpenAI users have the `OpenAICompatClient` + profile path as the clean route, and adding a parallel env-var hole would entrench the legacy path twice instead of once.

**UI — Playground prompt-template preview + plain-language case explainer**
- PR #57 (`wt/playground-prompt-preview`) — closes the cc-project eval UX gap surfaced right after PR #56 landed: a user clicking "Open in Playground" from a fully-resolved prompt's Inventory page lost all visibility into what the prompt actually said — the page rendered as "a form of empty boxes" with no `<pre>` block, no role labels, only a case-input table and an unfamiliar `dataset` field. New `src/aitap/ui/src/components/PromptPreviewCard.tsx` renders the system / user template text at the top of the Playground's right column whenever the target is a prompt (pipelines keep using `DagView`). Expanded by default — `<Card>` header with prompt name, source file + line number, an "Open in Inventory" link to `/prompts/{id}` for jumping into version history / diff, and a Hide/Show toggle (`aria-expanded` + `aria-controls`); body mirrors `PromptDetail.tsx`'s message list (per-message role badge + `template_kind` + variable names + `<pre>` text), with the PR #53 unresolved-text fallback wired per message (`<pre>aitap scan --deep {file}</pre>` instead of an empty box). Collapsed body shows a single-line truncated preview with a documented priority chain (`system` → `user` → `assistant` → `tool` → role-agnostic any-text fallback for forward compatibility with future scanner-emitted roles → localised `noResolvedText` hint, ≤ 120 chars + ellipsis) so the row never reads as empty. `CaseEditor` gets a plain-language subtitle in the Playground: en `Each case = one set of variable values. Hitting Run sends every case through the prompt once and shows you the answers side by side.` / zh `每个 case = 一组变量值。点 Run 会把所有 case 各跑一次,把答案并排显示。`. New i18n keys `promptPreview.title|subtitle|show|hide|openInInventory|noResolvedText` + `playground.caseExplanation` added to both `en.json` + `zh.json` (parity test passes, 7/7 keys both sides). +10 component unit tests (`src/aitap/ui/src/components/__tests__/PromptPreviewCard.test.tsx`): expanded default rendering of both messages; variable-name surfacing; PR #53 per-message unresolved fallback; Hide → collapsed `<span>` preview → Show round-trip; collapsed body still readable when every message is empty; header subtitle carrying `file:line`; **boundary cases pinning the role-priority + role-agnostic chain + open-detail link**: system-after-user source-order priority, non-canonical-role (`developer`) role-agnostic fallback, empty-messages array still renders header + open-detail link, header link href targets `/prompts/{id}`. Frontend tests: 60 → **70** passing across 14 → **15** test files. Backend untouched (909 passing, same as PR #56). Carries the i18n + plain-language UI copy rules from `CLAUDE.md`.

**Scanner — cross-call prompt resolution (builder body + wrapper link)**
- PR #56 (`wt/scanner-link-builder`) — closes the loop the cc-project eval surfaced after PRs #46 + #47 + #52: every agent file resolves to a `wrapper-call` site that reads `await self._llm.complete(messages, task_type=...)`, but `messages` is a Name reference and the per-site AST extractor can't see the literal text the builder produced. The new `src/aitap/scanner/rules/cross_call_resolution.py` adds two post-processing passes that run after the per-file scan. **Pass 1 — `upgrade_builder_message_lists`**: for each builder site with all UNRESOLVED messages, parse the function's `return [...]` list and resolve each item — `{"role": ..., "content": <Name>}` chases the Name back through up to four hops of local assignments → module-level constants → `extract_template` (so cc-project's `content = HEAVEN_WORLD_RULES → dedent(...)` chain inside `_system()` lands as real text); `_helper()` calls look up the helper in the same module and recurse into its return statement (one level, no chains). Sites the pass upgrades carry a `builder-body-resolved` tag. **Pass 2 — `link_wrapper_sites_to_builders`**: for each wrapper-call site with all UNRESOLVED messages, walk the enclosing function for `messages = build_xxx(...)` (or `messages: list = build_xxx(...)`, or `messages = await build_xxx(...)`), look up the builder by name in the project-global builder index, and copy its (now resolved) messages onto the wrapper. Sites the pass upgrades carry a `linked-from-builder` tag and keep their original id. Both passes are pure post-processing — sites they can't touch pass through unchanged, the pass order is deterministic, and a second run is a no-op (idempotent). What stays out of scope (documented in the module docstring): `messages = factory().build(...)` method-chain RHS, ternary RHS that picks one of two builders, multi-level helper chains, and per-method SDK wrapper paths that cross a class boundary — those need the L2 path or a follow-up rule. Verified against cc-project: **35 → 49 resolved messages / 60 → 74 total** (35 % → 66 % L1 resolution rate); **8 builder sites get full 2/2 system+user text** including `build_digest_messages` / `build_plan_messages` / `build_interaction_messages` / `build_reflection_messages` / `build_first_meet_story_messages` / `build_reunion_story_messages` / `build_location_discovery_messages` / `build_personality_messages`; **7 wrapper-call sites get the linked builder text** (`digest_generator.generate` / `interaction_engine.run_interaction` / `interaction_engine.update_relationship` / `memory_manager.score_importance` / `personality_builder.build` / `planner.plan_day` / `reflection_engine.reflect`). +13 unit tests (`tests/unit/test_scanner_cross_call_resolution.py`) covering builder upgrade (helper call + Name + multi-hop chain, idempotence, non-touch on already-resolved, non-touch on dynamic returns), wrapper link (same-file, cross-file, kwarg form, async + sync + await variants, non-touch on already-resolved, non-touch on factory method RHS), and a combined-pass smoke test that exercises both passes through `scan_project`. 893 → **906 backend**.
